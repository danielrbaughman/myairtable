...
- **`context.Context`** is the first argument of every I/O method: `at.Primary.Get(ctx, id)`, `rec.Save(ctx)`.
...
- **Typed table** layer uses generics: `at.Primary` is a `Table[PrimaryModel, *PrimaryModel]` (Get/List/Create/Update/Upsert/Delete + `.Dict()` for raw access). Per-model fluent `Save`/`Fetch`/`Delete` work on models bound via `at.Primary.Attach(&m)` or returned from the table.
...
- **Filtering**: `at.Primary.List(ctx, (&airtable.Query{}).WithFilterFormula(airtable.PrimaryF.PrimaryKey.Eq("alice@example.com")))`.
