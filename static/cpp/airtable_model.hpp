...
/// directly: `id`, `created_time`, `client_`, `snapshot_`, plus the two
/// generated hooks `collect_writable_fields()` / `collect_computed_fields()`
/// and the `kTableId` constant. Everything else — snapshotting, dirty diffing,
/// payload building, the fluent save/fetch/remove — lives here so the
/// generator emits no boilerplate.
///
