...
class CodeEmitter:
    ...
    def _emit_field_ref(self, node: FieldRef) -> str:
        ...
        if self.language in ("java", "csharp"):
            # Java/C# have no top-level functions — V() is a static on AirtableRuntime
    ...
    def _emit_fn_switch(self, node: FunctionCall) -> str:
        ...
        if self.language in ("java", "csharp"):
            # Java/C# AirtableRuntime.SWITCH(expr, default, flatPairs...) — varargs/params last.
            flat = ", ".join(part for pat, val in pairs for part in (pat, val))
            suffix = f", {flat}" if flat else ""
        ...
        if node.op in ("=", "!="):
            ...
            if self.language == "csharp":
                ...
                if "string" in (left_type, right_type):
                    return f"{rt}.V({rt}.S({left_raw}) {native_op} {rt}.S({right_raw}))"
                bang = "!" if native_op == "!=" else ""
                return f"{rt}.V({bang}{rt}.IsEqual({left_raw}, {right_raw}))"
            if self.language == "go":
