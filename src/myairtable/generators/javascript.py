...
# Member names a generated model may not use. A generated `get copy()` accessor
# on the subclass silently SHADOWS the `copy()` verb inherited from
# AirtableModel -- no error anywhere, and `record.copy()` raises
# `TypeError: record.copy is not a function` at the call site. Colliding field
# names get a `Field` suffix, then re-deduplicate.
#
# `id` / `createdTime` are not listed: they are pre-renamed upstream by
# `sanitize_reserved_names`.
_JAVASCRIPT_RESERVED_MODEL_MEMBERS = frozenset({"copy"})


def _field_property_map(table: Table) -> dict[str, str]:
    """`{field_id: deduplicated lowerCamelCase property name}` for one table.

    Every writer (models, Fields consts, Filters, and the transpiler's
    ...
    adjusted = [f"{name}Field" if name in _JAVASCRIPT_RESERVED_MODEL_MEMBERS else name for name in raw.values()]
