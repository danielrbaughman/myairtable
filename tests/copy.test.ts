...
import { AirtableModel, FieldDescriptor } from "../src/myairtable/static/typescript/airtable-model";
...
class TestModel extends AirtableModel<any, any, any> {
	protected static fieldDescriptors: FieldDescriptor[] = DESCRIPTORS;

	constructor(data: Record<string, unknown> = {}) {
		super((data?.id as string) ?? "");
	...
	public get dirtyFields(): string[] {
		return [...this._dirtyFields].sort();
	}
	public raw(name: string): any {
		return this._fields[name];
	}
	public createPayload(): Record<string, unknown> {
		return this.writableFields(true) as Record<string, unknown>;
	}
	/** Puts the model in the state a server read leaves it in. */
	public asServerRead(): this {
		this.clearDirtyFlags();
		return this;
	}
}

function source(overrides: Record<string, unknown> = {}): TestModel {
