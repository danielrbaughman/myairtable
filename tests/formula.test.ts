...
describe("SingleSelectField", () => {
	describe("equals", () => {
		it("delegates to TextField.equals", () => {
			const field = new SingleSelectField<string>("Status");
...
describe("DateField", () => {
	describe("on", () => {
		...
		it("with date returns formula string", () => {
			const field = new DateField("Created");
			const result = field.on("2024-01-15");
...
describe("DateComparison", () => {
	...
	const units = ["milliseconds", "seconds", "minutes", "hours", "days", "weeks", "months", "quarters", "years"];

	timeAgoMethods.forEach((method, index) => {
		describe(method, () => {
			it(`produces DATETIME_DIFF with ${units[index]}`, () => {
				const field = new DateField("Created");
				const comparison = field.onOrAfter();
				const result = comparison[method](10);
				expect(result).toContain("DATETIME_DIFF");
				expect(result).toContain(units[index]);
			});
		});
	});
});

// TypeScript-specific tests for generics
describe("TypeScript Generics", () => {
	describe("SingleSelectField<T>", () => {
		it("accepts generic type parameter", () => {
			type StatusOptions = "Active" | "Inactive" | "Pending";
			const field = new SingleSelectField<StatusOptions>("Status");
			// TypeScript compiler ensures only valid options
			expect(field.equals("Active")).toContain("Active");
		});

		it("works with string literal union types", () => {
			type Priority = "High" | "Medium" | "Low";
			const field = new SingleSelectField<Priority>("Priority");
			expect(field.equals("High")).toBe('{Priority}="High"');
			expect(field.notEquals("Low")).toBe('{Priority}!="Low"');
		});
	});

	describe("MultiSelectField<T>", () => {
		it("accepts generic type parameter", () => {
			type TagOptions = "Tag1" | "Tag2" | "Tag3";
			const field = new MultiSelectField<TagOptions>("Tags");
			expect(field.containsAllOptions(["Tag1", "Tag2"])).toContain("AND(");
		});

		it("containsAnyOptions works with typed array", () => {
			type Categories = "Tech" | "Business" | "Design";
			const field = new MultiSelectField<Categories>("Categories");
			const result = field.containsAnyOptions(["Tech", "Design"]);
			expect(result).toContain("OR(");
		});

		it("notContainsOptions works with typed array", () => {
			type Features = "Dark Mode" | "Light Mode" | "Auto";
			const field = new MultiSelectField<Features>("Features");
			const result = field.notContainsOptions(["Dark Mode", "Light Mode"]);
			expect(result).toContain("AND(");
		});
	});
});

// Test type inference and method chaining
describe("Method Chaining", () => {
	it("DateComparison can be chained from DateField", () => {
		const field = new DateField("Created");
		const result = field.onOrAfter().daysAgo(7);
		expect(result).toContain("DATETIME_DIFF");
		expect(result).toContain("days");
	});

	it("multiple DateComparison chains work independently", () => {
		const field = new DateField("Modified");
		const comparison1 = field.on().hoursAgo(24);
		const comparison2 = field.onOrBefore().weeksAgo(2);

		expect(comparison1).toContain("hours");
		expect(comparison2).toContain("weeks");
	});
});
...
describe("Complex Formula Integration", () => {
	describe("multi-field AND combinations", () => {
		it("combines text, number, and date fields", () => {
			...
			const result = AND(name.contains("test"), amount.greaterThan(100), created.onOrAfter("2024-01-01") as string);
	...
	describe("multi-field OR combinations", () => {
		it("combines select fields and boolean", () => {
			type StatusOptions = "Active" | "Pending" | "Closed";
			const status = new SingleSelectField<StatusOptions>("Status");
	...
	describe("MultiSelect complex conditions", () => {
		it("combines contains_all with NOT contains", () => {
			type TagOptions = "urgent" | "reviewed" | "archived";
			const tags = new MultiSelectField<TagOptions>("Tags");
	...
	describe("phone normalization with other conditions", () => {
		it("combines phoneEquals with status and date filters", () => {
			...
			const result = AND(
				phone.phoneEquals("555-123-4567"),
				status.equals("Active"),
				created.onOrAfter("2024-01-01") as string,
			);
	...
	describe("attachment field in complex formulas", () => {
		...
		it("combines notEmpty attachments with date filter", () => {
			...
			const result = AND(docs.notEmpty(), submitted.onOrBefore("2024-06-30") as string);
	...
	describe("four-level deep nesting", () => {
		it("handles deeply nested logic", () => {
			...
			const priority = new NumberField("Priority");
			const active = new BooleanField("Active");
			type StatusOptions = "Urgent" | "Normal";
			const status = new SingleSelectField<StatusOptions>("Status");
	...
	describe("exclusive vs inclusive ranges", () => {
		it("combines exclusive number range with date filter", () => {
			...
			const result = AND(amount.between(100, 500, false), date.onOrAfter("2024-01-01") as string);
